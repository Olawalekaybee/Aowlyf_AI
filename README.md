...
**Phase 2: Staff and folder provisioning** (done)
See `scripts/`:
...
is enforced at the application layer: the MCP server and future file-serving
...
**Phase 3: Web app, dashboard and Gantt** (done)
...
- `webapp/backend/`: a small FastAPI app with `/auth/login` (checks the
...
- `webapp/frontend/`: a React and Vite dashboard with a login screen, a project
...
delete it). No projects exist yet fresh out of Phase 2, so either create one
...
The dashboard also covers project/task management and the request
pipeline directly, not just via Claude or the MCP tools:

- **Create project** (sidebar `+ New`): any staff member can propose one;
  admin-created projects go straight to `active`, everyone else's start as
  `proposed` until an admin activates them from the sidebar.
- **Create task** (`+ New task` on a selected project): shows up on the
  Gantt immediately over the shared WebSocket connection.
- **Add team member** (`+ Add member`): restricted to admins or staff
  explicitly flagged `can_grant_team_membership`, matching the MCP
  permission rule exactly.
- **Concerns** (top nav): any staff member can raise one against a
  category (project issue, work concern, procurement related, general
  update, internal review), optionally tied to a project. Admins see the
  full queue with filtering by status and can resolve one inline with a
  written response.
- **Procurement** (top nav): any staff member can submit an item request
  with quantity, justification, and an optional project link. Admins see
  the full queue and move each request through
  `requested → approved → ordered → received` (or `rejected`) from a
  dropdown.

Both queues also expose a personal view for non-admins
